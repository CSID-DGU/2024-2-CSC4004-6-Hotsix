...
function Bingo() {
  ...
  useEffect(() => {
    // recommendedCourses 불러오기
    const courses = JSON.parse(sessionStorage.getItem('recommendedCourses')) || [];

    // courses를 기반으로 GPT에 활동 추천 요청
    // 예: prompt: "다음 장소들에 대해 각각 할만한 재미있는 활동을 한 문장씩 총 9개 추천해줘: '경복궁', '남산타워', ..."

    const prompt = `
      내가 아래 장소들에 대해 재미있는 데이트 활동을 한 문장씩 9개만 말해줘.
      장소 목록: ${courses.join(', ')}
      출력은 장소명 + "에서" + "할 활동" 형태로 각 줄에 하나씩.
      다른 내용 없이 오직 활동 목록만.
    `;
