...
//
                        //여기는 윈도우 공통 경로
//						"srㅕc\\main\\resources\\static\\asset\\Images\\postImage\\";
                        //여기는 mac 공통 경로
...
//        UserRep.save(user);
    ...
    public ResponseEntity<?> getUserDetail(@PathVariable String userId){
        Optional<UserDomain> optUser = userRepository.findById(userId);
        UserDomain user = optUser.get();

        return ResponseEntity.ok().body(Map.of(
                "activityPreference", String.valueOf(user.getActivityPreference()),
                "dayBudgetRange",user.getDayBudgetRange(),
                "requiredLocation",user.getRequiredLocation(),
                "mbti",user.getMbti(),
                "preferredCourse",user.getPreferredCourse(),
                "requiredCourse",user.getRequiredCourse(),
                "startTime",user.getStartTime(),
                "transportType",user.getTransportType(),
